            ...
            if (Window.confirm(new ClientMessage("", "确认创建新内存设备").toString())) {
                if (memoryAddView == null) {
                    ...
                    memoryAddView.setPresenter(new DeviceMemoryAddView.Presenter() {
                        ...
                        @Override
                        public boolean onOK(String memory_name, String memory_desc, String size, String server_name) {
                            ...
                            try {
                                if (!isEmpty(size)) {
                                    memory_size = Long.parseLong(size);
                                }
                            }
                            catch (Exception e) {
                            }
                            if (memory_size <= 0) {
                                StringBuilder sb = new StringBuilder();
                                sb.append(new ClientMessage("", "内存大小非法")).append(" = '").append(size).append("' ");
            ...
            if (Window.confirm(new ClientMessage("", "确认修改所选择的内存").toString())) {
                ...
                if (memoryModifyView == null) {
                    ...
                    memoryModifyView.setPresenter(new DeviceMemoryModifyView.Presenter() {
                        ...
                        @Override
                        public boolean onOK(int memory_id, String memory_desc, String size) {
                            long memory_size = 0;
                            try {
                                if (!isEmpty(size)) {
                                    memory_size = Long.parseLong(size);
                ...
                memoryModifyView.popup(Integer.parseInt(row.getField(CellTableColumns.MEMORY.MEMORY_ID)), row.getField(CellTableColumns.MEMORY.MEMORY_NAME),
                        row.getField(CellTableColumns.MEMORY.MEMORY_DESC), Long.parseLong(row.getField(CellTableColumns.MEMORY.MEMORY_TOTAL)), row.getField(CellTableColumns.MEMORY.SERVER_NAME));
            }
        }
        catch (Exception e) {
            e.printStackTrace();
            ...
            if (Window.confirm(new ClientMessage("", "确认添加的内存服务").toString())) {
                ...
                if (memoryServiceAddView == null) {
                    ...
                    memoryServiceAddView.setPresenter(new DeviceMemoryServiceAddView.Presenter() {
                        ...
                        @Override
                        public boolean onOK(int memory_id, String ms_desc, String used, String starttime, String endtime, String account_name, String user_name) {
                            long ms_used = 0;
                            try {
                                if (!isEmpty(used)) {
                                    ms_used = Long.parseLong(used);
                                }
                            }
                            catch (Exception e) {
                            }
                            ...
                            try {
                                if (!isEmpty(starttime)) {
                                    ms_starttime = DeviceDateBox.parse(starttime);
                                }
                            }
                            catch (Exception e) {
                            }
                            ...
                            try {
                                if (!isEmpty(endtime)) {
                                    ms_endtime = DeviceDateBox.parse(endtime);
                                }
                            }
                            catch (Exception e) {
                            }
                            ...
                            if (ms_starttime.getTime() >= ms_endtime.getTime()) {
                                StringBuilder sb = new StringBuilder();
                                sb.append(new ClientMessage("", "非法的服务时间")).append(" = '").append(ms_starttime).append("' >= '").append(ms_endtime).append("'");
                                sb.append(new ClientMessage("", "请重新选择时间"));
                                Window.alert(sb.toString());
                                return false;
                            }
                ...
                memoryServiceAddView.popup(Integer.parseInt(row.getField(CellTableColumns.MEMORY.MEMORY_ID)), row.getField(CellTableColumns.MEMORY.MEMORY_NAME),
                        row.getField(CellTableColumns.MEMORY.SERVER_NAME), Long.parseLong(row.getField(CellTableColumns.MEMORY.MEMORY_SERVICE_USED)));
            }
        }
        catch (Exception e) {
            e.printStackTrace();
            ...
            if (Window.confirm(new ClientMessage("", "确认修改所选择的内存服务").toString())) {
                ...
                if (memoryServiceModifyView == null) {
                    ...
                    memoryServiceModifyView.setPresenter(new DeviceMemoryServiceModifyView.Presenter() {
                        ...
                        @Override
                        public boolean onOK(int ms_id, String ms_desc, String starttime, String endtime) {
                            Date ms_starttime = null;
                            try {
                                if (!isEmpty(starttime)) {
                                    ms_starttime = DeviceDateBox.parse(starttime);
                ...
                memoryServiceModifyView.popup(Integer.parseInt(row.getField(CellTableColumns.MEMORY.MEMORY_SERVICE_ID)), row.getField(CellTableColumns.MEMORY.MEMORY_NAME), row.getField(CellTableColumns.MEMORY.MEMORY_DESC),
                        Long.parseLong(row.getField(CellTableColumns.MEMORY.MEMORY_SERVICE_USED)), row.getField(CellTableColumns.MEMORY.MEMORY_SERVICE_STARTTIME), row.getField(CellTableColumns.MEMORY.MEMORY_SERVICE_ENDTIME),
                        row.getField(CellTableColumns.MEMORY.SERVER_NAME), row.getField(CellTableColumns.MEMORY.ACCOUNT_NAME), row.getField(CellTableColumns.MEMORY.USER_NAME));
            }
        }
        catch (Exception e) {
            e.printStackTrace();
