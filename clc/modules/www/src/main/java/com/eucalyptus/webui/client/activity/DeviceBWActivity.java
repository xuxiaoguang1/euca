...
import com.eucalyptus.webui.server.DeviceBWServiceProcImpl;
...
public class DeviceBWActivity extends AbstractSearchActivity implements DeviceBWView.Presenter {
	...
	private void prepareModifyService(SearchResultRow row) {
		...
		String life;
		if (isSystemAdmin) {
			starttime = row.getField(DeviceBWServiceProcImpl.TABLE_COL_INDEX_ROOT_STARTTIME);
			life = row.getField(DeviceBWServiceProcImpl.TABLE_COL_INDEX_ROOT_LIFE);
		}
		else {
			        ...
			        public void onSuccess(SearchResultRow result) {
				        if (result != null) {
					        ...
					        if (getView().isMirrorMode()) {
						        final int col = DeviceBWServiceProcImpl.TABLE_COL_INDEX_BS_ID;
						        result.setField(DeviceBWServiceProcImpl.TABLE_COL_INDEX_CHECKBOX, "+");
	...
	private SearchResultRow copyRow(SearchResultRow row) {
		...
		tmp.setField(DeviceBWServiceProcImpl.TABLE_COL_INDEX_CHECKBOX, "");
		return tmp;
	}
	...
	private boolean hasService(SearchResultRow row) {
		final int col;
		if (isSystemAdmin) {
			col = DeviceBWServiceProcImpl.TABLE_COL_INDEX_ROOT_STARTTIME;
		}
		else {
	...
	private List<SearchResultRow> sortSearchResultRow(List<SearchResultRow> list) {
		Collections.sort(list, new Comparator<SearchResultRow>() {
			...
			public int compare(SearchResultRow arg0, SearchResultRow arg1) {
				String v0 = arg0.getField(DeviceBWServiceProcImpl.TABLE_COL_INDEX_NO);
				String v1 = arg1.getField(DeviceBWServiceProcImpl.TABLE_COL_INDEX_NO);
				if (v0 == null) {
					return v1 == null ? 0 : 1;
				}
				if (v1 == null) {
