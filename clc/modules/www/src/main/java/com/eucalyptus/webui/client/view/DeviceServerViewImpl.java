...
public class DeviceServerViewImpl extends Composite implements DeviceServerView {
	...
	private static ServerViewImplUiBinder uiBinder = GWT.create(ServerViewImplUiBinder.class);
	
	interface ServerViewImplUiBinder extends UiBinder<Widget, DeviceServerViewImpl> {
	}
	...
	Anchor[] labelList = {labelAll, labelStop, labelInuse, labelError};
	...
	void handleLabelAll(ClickEvent event) {
		labelAll.setHTML("全部服务器数量：<font color='red'>" + numAll + "</font> 台");
	}
	
	@UiHandler("labelStop")
	void handleLabelStop(ClickEvent event) {
		labelStop.setHTML("停止中服务器数量：" + numStop + " 台");
	}
	
	@UiHandler("labelInuse")
	void handleLabelInuse(ClickEvent event) {
		labelInuse.setHTML("使用中服务器数量：" + numInuse + " 台");
	}
	
	@UiHandler("labelError")
	void handleLabelError(ClickEvent event) {
		labelError.setHTML("故障中服务器数量：" + numError + " 台");
	}
	
//	private void updateButtonError(int n) {
//		ButtonError.setHTML("故障中服务器数量：<font color='red'>" + n +"</font>");
//	}
//	
