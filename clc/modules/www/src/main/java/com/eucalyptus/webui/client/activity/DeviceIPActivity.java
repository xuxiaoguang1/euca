...
import com.eucalyptus.webui.server.DeviceIPServiceProcImpl;
...
public class DeviceIPActivity extends AbstractSearchActivity implements DeviceIPView.Presenter {
	...
	private void prepareAddService(SearchResultRow row) {
		...
		String life;
		if (isSystemAdmin) {
			starttime = row.getField(DeviceIPServiceProcImpl.TABLE_COL_INDEX_ROOT_STARTTIME);
			state = row.getField(DeviceIPServiceProcImpl.TABLE_COL_INDEX_ROOT_STATE);
			life = row.getField(DeviceIPServiceProcImpl.TABLE_COL_INDEX_ROOT_LIFE);
		}
		else {
			starttime = row.getField(DeviceIPServiceProcImpl.TABLE_COL_INDEX_USER_STARTTIME);
			        ...
			        public void onSuccess(SearchResultRow result) {
				        if (result != null) {
					        ...
					        if (getView().isMirrorMode()) {
						        final int col = DeviceIPServiceProcImpl.TABLE_COL_INDEX_IS_ID;
						        result.setField(DeviceIPServiceProcImpl.TABLE_COL_INDEX_CHECKBOX, "+");
		...
		getBackendService().deleteDeviceIPDevice(getSession(), list, new AsyncCallback<List<SearchResultRow>>() {
			...
			public void onSuccess(List<SearchResultRow> result) {
				if (result != null) {
					...
					if (getView().isMirrorMode()) {
						final int col = DeviceIPServiceProcImpl.TABLE_COL_INDEX_IP_ID;
						final SearchResultRowMatcher matcher = new SearchResultRowMatcher() {
	...
	private SearchResultRow copyRow(SearchResultRow row) {
		...
		tmp.setField(DeviceIPServiceProcImpl.TABLE_COL_INDEX_CHECKBOX, "");
		return tmp;
	}
	...
	private boolean hasService(SearchResultRow row) {
		final int col;
		if (isSystemAdmin) {
			col = DeviceIPServiceProcImpl.TABLE_COL_INDEX_ROOT_STARTTIME;
		}
		else {
	...
	private List<SearchResultRow> sortSearchResultRow(List<SearchResultRow> list) {
		Collections.sort(list, new Comparator<SearchResultRow>() {
			...
			public int compare(SearchResultRow arg0, SearchResultRow arg1) {
				String v0 = arg0.getField(DeviceIPServiceProcImpl.TABLE_COL_INDEX_NO);
				String v1 = arg1.getField(DeviceIPServiceProcImpl.TABLE_COL_INDEX_NO);
				if (v0 == null) {
					return v1 == null ? 0 : 1;
				}
				if (v1 == null) {
