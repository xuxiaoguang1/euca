...
public class DeviceMemoryActivity extends AbstractSearchActivity implements DeviceMemoryView.Presenter {
	...
	private void prepareAddService(SearchResultRow row) {
		...
		String used;
		if (isSystemAdmin) {
			starttime = row.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_ROOT_STARTTIME);
			state = row.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_ROOT_STATE);
			life = row.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_ROOT_LIFE);
			used = row.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_USED);
		}
		else {
			starttime = row.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_USER_STARTTIME);
			state = row.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_USER_STATE);
			life = row.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_USER_LIFE);
			used = row.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_USED);
		}
			        ...
			        public void onSuccess(SearchResultRow result) {
				        if (result != null) {
					        ...
					        if (getView().isMirrorMode()) {
						        final int col = DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_MS_ID;
						        result.setField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_CHECKBOX, "+");
						        ...
						        long used = 0;
						        try {
						        	used = Long.parseLong(result.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_USED));
						        }
						        catch (Exception e) {
						        }
						        if (used != 0) {
		...
		getBackendService().deleteDeviceMemoryDevice(getSession(), list, new AsyncCallback<List<SearchResultRow>>() {
			...
			public void onSuccess(List<SearchResultRow> result) {
				if (result != null) {
					...
					if (getView().isMirrorMode()) {
						final int col = DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_MEM_ID;
						final SearchResultRowMatcher matcher = new SearchResultRowMatcher() {
	...
	private SearchResultRow copyRow(SearchResultRow row) {
		...
		tmp.setField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_CHECKBOX, "");
		return tmp;
	}
	...
	private boolean hasService(SearchResultRow row) {
		final int col;
		if (isSystemAdmin) {
			col = DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_ROOT_STARTTIME;
		}
		else {
			col = DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_USER_STARTTIME;
		}
	...
	public void onDeleteDevice() {
		...
		for (SearchResultRow row : selected) {
			if (!hasService(row)) {
				String used = row.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_USED);
				String total = row.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_TOTAL);
				try {
	...
	private List<SearchResultRow> sortSearchResultRow(List<SearchResultRow> list) {
		Collections.sort(list, new Comparator<SearchResultRow>() {
			...
			public int compare(SearchResultRow arg0, SearchResultRow arg1) {
				String v0 = arg0.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_NO);
				String v1 = arg1.getField(DeviceMemoryServiceProcImpl.TABLE_COL_INDEX_NO);
				if (v0 == null) {
					return v1 == null ? 0 : 1;
				}
				if (v1 == null) {
