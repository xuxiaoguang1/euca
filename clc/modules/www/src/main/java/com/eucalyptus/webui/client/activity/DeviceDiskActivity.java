...
public class DeviceDiskActivity extends AbstractSearchActivity implements DeviceDiskView.Presenter {
	...
	private static final Logger LOG = Logger.getLogger(DeviceDiskActivity.class.getName());

	public static final String TITLE[] = {"Disk", "硬盘"};
	...
	private int queryState = DiskState.getInvalidValue();

	@Override
	protected void doSearch(String query, SearchRange range) {
		getBackendService().lookupDeviceDisk(getSession(), query, range, queryState, new AsyncCallback<SearchResult>() {
	...
	public static class DiskState {
		...
		final private static DiskState[] ALL_STATES = {new DiskState(State.START), new DiskState(State.STOP),
		        new DiskState(State.RESERVED)};

		public static DiskState getDiskState(int state) {
			for (DiskState s : ALL_STATES) {
				if (s.getValue() == state) {
					return s;
				}
			}
