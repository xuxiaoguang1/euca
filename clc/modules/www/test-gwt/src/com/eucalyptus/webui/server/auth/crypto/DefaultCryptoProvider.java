...
import com.eucalyptus.webui.server.auth.util.Hashes;
...
public class DefaultCryptoProvider implements CryptoProvider,
	...
	public String generateQueryId() {
		return generateRandomAlphanumeric(21).toUpperCase();// NOTE: this MUST
															// be 21-alnums
															// upper case.
	}
	...
	public String generateSecretKey() {
		return generateRandomAlphanumeric(40);// NOTE: this MUST be 40-chars
												// from base64.
	}
	 ...
	 * 
	 * <p>
	 * The output has the following substitutions:
	 * </p>
	 * 
	 * <ul>
	 * <li>+ -> -</li>
	 ...
	 * </ul>
	 * 
	 * @see com.eucalyptus.crypto.CryptoProvider#getDigestBase64(java.lang.String,
	 *      com.eucalyptus.crypto.Digest)
	 */
	@Override
	...
	public X509Certificate generateServiceCertificate(KeyPair keys,
			String serviceName) {
		X500Principal x500 = new X500Principal(String.format(
				"CN=%s, OU=Eucalyptus, O=Cloud, C=US", serviceName));
		// if( !"eucalyptus".equals( serviceName ) ) {
		// SystemCredentials sys = SystemCredentials.lookup( Eucalyptus.class );
		// return generateCertificate( keys, x500, sys.getCertificate(
		// ).getSubjectX500Principal( ), sys.getPrivateKey( ) );
		// } else {
	...
	public X509Certificate generateCertificate(KeyPair keys, String userName) {
		return generateCertificate(
				keys,
				new X500Principal(String.format(
						"CN=%s, OU=Eucalyptus, O=User, C=US", userName)));
	}
	...
	public X509Certificate generateCertificate(KeyPair keys,
			X500Principal subjectDn, X500Principal signer, PrivateKey signingKey) {
		signer = (signingKey == null ? signer : subjectDn);
		signingKey = (signingKey == null ? keys.getPrivate() : signingKey);
		// EventRecord.caller( DefaultCryptoProvider.class,
		// EventType.GENERATE_CERTIFICATE, signer.toString( ),
	...
	public String generateId(final String seed, final String prefix) {
		...
		for (int i = key.length(); i < 128; i += 8) {
			hash.update(Longs.toByteArray(Double.doubleToRawLongBits(Math
					.random())));
		}
