...
//		keyDBProc = new AccessKeyDBProcWrapper();
...
//			}
//
//			public SearchResult lookupCertificate(LoginUserProfile curUser,
//					String search, SearchRange range) throws EucalyptusServiceException {
...
//	}
	...
	private SearchResult getSearchResult(boolean isRootAdmin,
			ResultSetWrapper rs, SearchRange range) {
		assert (range != null);
	...
	private List<SearchResultRow> resultSet2List(boolean isRootView,
		...
		try {
			if (rs != null) {
				...
				while (rs.next()) {
					boolean userKeyActive = Integer.parseInt(rs
							.getString(DBTableColName.USER_KEY.ACTIVE)) == 1;

					if (isRootView) {
						result.add(new SearchResultRow(
								Arrays.asList(
										rs.getString(DBTableColName.USER_KEY.ID),
										Integer.toString(index++),
										Enum2String.getInstance()
												.getActiveState(
														userKeyActive),
...
//	}
	...
	private static final String[] TABLE_COL_TITLE_CHECKALL = { "Check All",
			"全选" };
	private static final String[] TABLE_COL_TITLE_NO = { "No.", "序号" };
	private static final String[] TABLE_COL_TITLE_KEY_ACTIVE = { "Active",
			"密钥状态" };
	private static final String[] TABLE_COL_TITLE_KEY_CREATED_DATE = {
			"Created Date", "创建时间" };
	private static final String[] TABLE_COL_TITLE_ACCOUNT_NAME = { "Account",
			"账户" };
	...
	private static final List<SearchResultFieldDesc> FIELDS_ROOT = Arrays
			.asList(new SearchResultFieldDesc(TABLE_COL_TITLE_CHECKALL[1],
					"10%", false), new SearchResultFieldDesc(
					TABLE_COL_TITLE_NO[1], false, "10%",
					TableDisplay.MANDATORY, Type.TEXT, false, false),
					new SearchResultFieldDesc(TABLE_COL_TITLE_KEY_ACTIVE[1],
							true, "10%", TableDisplay.MANDATORY, Type.TEXT,
							false, false), new SearchResultFieldDesc(
							TABLE_COL_TITLE_KEY_CREATED_DATE[1], true, "25%",
							TableDisplay.MANDATORY, Type.TEXT, false, false),
					new SearchResultFieldDesc(TABLE_COL_TITLE_ACCOUNT_NAME[1],
							true, "15%", TableDisplay.MANDATORY, Type.TEXT,
							false, false), new SearchResultFieldDesc(
							TABLE_COL_TITLE_GROUP_NAME[1], true, "15%",
							TableDisplay.MANDATORY, Type.TEXT, false, false),
					new SearchResultFieldDesc(TABLE_COL_TITLE_NAME[1], true,
							"15%", TableDisplay.MANDATORY, Type.TEXT, false,
							false));
	...
	private static final List<SearchResultFieldDesc> FIELDS_NONROOT = Arrays
			.asList(new SearchResultFieldDesc(TABLE_COL_TITLE_CHECKALL[1],
					...
					new SearchResultFieldDesc(TABLE_COL_TITLE_KEY_ACTIVE[1],
							true, "10%", TableDisplay.MANDATORY, Type.TEXT,
							false, false), new SearchResultFieldDesc(
							TABLE_COL_TITLE_KEY_CREATED_DATE[1], true, "30%",
							TableDisplay.MANDATORY, Type.TEXT, false, false),
					new SearchResultFieldDesc(TABLE_COL_TITLE_GROUP_NAME[1],
							true, "20%", TableDisplay.MANDATORY, Type.TEXT,
							false, false), new SearchResultFieldDesc(
							TABLE_COL_TITLE_NAME[1], true, "20%",
							TableDisplay.MANDATORY, Type.TEXT, false, false));
