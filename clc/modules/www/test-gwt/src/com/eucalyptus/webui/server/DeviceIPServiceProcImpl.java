...
class DeviceIPDBProcWrapper {
	 ...
	 * and user.id=$user_id
	 */
	ResultSetWrapper queryAllIPs(int queryState, int queryType, int user_id) throws Exception {
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT * FROM ");
		sb.append(DBTableName.IP).append(" LEFT JOIN ").append(DBTableName.IP_SERVICE);
		sb.append(" ON ");
	...
	/* *
	 * update ip_service set is_life=(select datediff((select if($endtime >
	 * is_starttime, $endtime, is_starttime)), is_starttime)),
	 * is_state=$is_state where is_id=$is_id and user_id=$user_id
	 */
	void modifyService(int is_id, int user_id, String endtime, int is_state) throws Exception {
		StringBuilder sb = new StringBuilder();
		sb.append("UPDATE ");
		sb.append(DBTableName.IP_SERVICE);
		sb.append(" SET ");
		...
		if (user_id >= 0) {
			sb.append(" AND ");
			sb.append(DBTableColName.IP_SERVICE.USER_ID).append(" = ").append(user_id);
		}
...
public class DeviceIPServiceProcImpl {
	...
	public SearchResult lookupIP(Session session, String search, SearchRange range, int queryState, int queryType) {
		...
		try {
			...
			if (isRoot) {
				rsw = dbproc.queryAllIPs(queryState, queryType, -1);
			}
			else {
				rsw = dbproc.queryAllIPs(queryState, queryType, getUser(session).getUserId());
			}
	...
	public SearchResultRow modifyService(Session session, SearchResultRow row, String sendtime, int state) {
		try {
			...
			modifyService(is_id, user_id, formatter.format(formatter.parse(sendtime)), state);
			return lookupService(row, isRoot, is_id);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	private SearchResultRow lookupService(SearchResultRow row, boolean isRoot, int is_id) throws Exception {
	...
	public SearchResultRow addService(Session session, SearchResultRow row, String account, String user,
		...
		try {
			...
			row.setField(TABLE_COL_INDEX_ROOT_STATE, IPState.getIPState(state).toString());
			return row;
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	private void modifyService(int is_id, int user_id, String endtime, int state) throws Exception {
		dbproc.modifyService(is_id, user_id, endtime, state);
	}
	...
	private SearchResultRow convertUserResultRow(ResultSet rs, int index) throws SQLException {
		...
		return new SearchResultRow(Arrays.asList(rs.getString(DBTableColName.IP_SERVICE.ID),
		        rs.getString(DBTableColName.IP.ID), "", Integer.toString(index),
		        rs.getString(DBTableColName.IP.ADDR), stype,
		        rs.getString(DBTableColName.VM.MARK), sstarttime, slife, sremains, sstate));
	}
