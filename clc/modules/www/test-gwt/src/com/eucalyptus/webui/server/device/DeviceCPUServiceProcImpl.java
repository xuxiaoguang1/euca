...
class DeviceCPUDBProcWrapper {
	...
	ResultSetWrapper queryAllCPUs(int queryState, int account_id, int user_id) throws Exception {
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT * FROM ");
...
public class DeviceCPUServiceProcImpl {
	...
	public SearchResult lookupCPU(Session session, String search, SearchRange range, int queryState) {
		...
		try {
			...
			if (user.isSystemAdmin()) {
				rsw = dbproc.queryAllCPUs(queryState, -1, -1);
			}
			else if (user.isAccountAdmin()) {
				rsw = dbproc.queryAllCPUs(queryState, account_id, -1);
			}
			else {
				rsw = dbproc.queryAllCPUs(queryState, account_id, user_id);
			}
	...
	public SearchResultRow modifyService(Session session, SearchResultRow row, String sendtime, int state) {
		try {
			...
			modifyService(cs_id, formatter.format(formatter.parse(sendtime)), state);
			return lookupService(row, cs_id);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void updateServiceState(int cs_id, int cs_state) throws EucalyptusServiceException {
		try {
			dbproc.updateServiceState(cs_id, cs_state);
		}
		catch (Exception e) {
			e.printStackTrace();
			throw new EucalyptusServiceException(new ClientMessage("", "更新CPU状态失败"));
		}
	}
	...
	public SearchResultRow addService(Session session, SearchResultRow row, String account, String user,
		...
		try {
			...
			row.setField(TABLE_COL_INDEX_STATE, CPUState.getCPUState(state).toString());
			return row;
		}
		catch (Exception e) {
			e.printStackTrace();
	...
	private void modifyService(int cs_id, String endtime, int state) throws Exception {
		dbproc.modifyService(cs_id, endtime, state);
	}
	...
	public static final int TABLE_COL_INDEX_ACCOUNT = 10;
	public static final int TABLE_COL_INDEX_USER = 11;
	public static final int TABLE_COL_INDEX_STARTTIME = 12;
	public static final int TABLE_COL_INDEX_LIFE = 13;
	public static final int TABLE_COL_INDEX_REMAINS = 14;
	public static final int TABLE_COL_INDEX_STATE = 15;
