...
class DeviceMemoryDBProcWrapper {
	...
	/* *
	 * select * from memory left join server on memory.server_id = server.server_id left join mem_service on
	 * memory.mem_id=mem_service.mem_id left join user on mem_service.user_id=user.user_id left join account on
	 * user.account_id=account.account_id where 1=1 and mem_service.ms_state=$queryState and user.id=$user_id
	 */
	ResultSetWrapper queryAllMemorys(int queryState, int user_id) throws Exception {
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT * FROM ");
	...
	/* *
	 * update mem_service set ms_life=(select datediff((select if($endtime >
	 * ms_starttime, $endtime, ms_starttime)), ms_starttime)),
	 * ms_state=$ms_state where ms_id=$ms_id and user_id=$user_id
	 */
	void modifyService(int ms_id, int user_id, String endtime, int ms_state) throws Exception {
		StringBuilder sb = new StringBuilder();
		sb.append("UPDATE ");
		sb.append(DBTableName.MEM_SERVICE);
		sb.append(" SET ");
		...
		if (user_id >= 0) {
			sb.append(" AND ");
			sb.append(DBTableColName.MEM_SERVICE.USER_ID).append(" = ").append(user_id);
		}
...
public class DeviceMemoryServiceProcImpl {
	...
	public SearchResult lookupMemory(Session session, String search, SearchRange range, int queryState) {
		...
		try {
			...
			if (isRoot) {
				rsw = dbproc.queryAllMemorys(queryState, -1);
			}
			else {
				rsw = dbproc.queryAllMemorys(queryState, getUser(session).getUserId());
			}
	...
	public SearchResultRow modifyService(Session session, SearchResultRow row, String sendtime, int state) {
		try {
			...
			modifyService(ms_id, user_id, formatter.format(formatter.parse(sendtime)), state);
			return lookupService(row, isRoot, ms_id);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	private SearchResultRow lookupService(SearchResultRow row, boolean isRoot, int ms_id) throws Exception {
	...
	public SearchResultRow addService(Session session, SearchResultRow row, String account, String user,
		...
		try {
			...
			return lookupService(row, true, ms_id);
		}
		catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void modifyService(int ms_id, int user_id, String endtime, int state) throws Exception {
		dbproc.modifyService(ms_id, user_id, endtime, state);
	}
		...
		if (rsw != null) {
			...
			try {
				...
				while (rs.next()) {
					SearchResultRow row;
					if (isRoot) {
						row = convertRootResultRow(rs, index);
					}
					else {
						row = convertUserResultRow(rs, index);
					}
					if (row != null) {
						index ++;
						rows.add(row);
	...
	public static final long DIV_UNIT = 1024;
