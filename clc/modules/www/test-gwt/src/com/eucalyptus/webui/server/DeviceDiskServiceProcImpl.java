...
class DeviceDiskDBProcWrapper {
	...
	/* *
	 * select * from disk left join server on disk.server_id = server.server_id left join disk_service on
	 * disk.disk_id=disk_service.disk_id left join user on disk_service.user_id=user.user_id left join account on
	 * user.account_id=account.account_id where 1=1 and disk_service.ms_state=$queryState and user.id=$user_id
	 */
	ResultSetWrapper queryAllDisks(int queryState, int user_id) throws Exception {
		StringBuilder sb = new StringBuilder();
		sb.append("SELECT * FROM ");
	...
	/* *
	 * update disk_service set ms_life=(select datediff((select if($endtime >
	...
	void modifyService(int ms_id, int user_id, String endtime, int ms_state) throws Exception {
		...
		if (user_id >= 0) {
			sb.append(" AND ");
			sb.append(DBTableColName.DISK_SERVICE.USER_ID).append(" = ").append(user_id);
		}
...
public class DeviceDiskServiceProcImpl {
	...
	public SearchResult lookupDisk(Session session, String search, SearchRange range, int queryState) {
		...
		try {
			...
			if (isRoot) {
				rsw = dbproc.queryAllDisks(queryState, -1);
			}
			else {
				rsw = dbproc.queryAllDisks(queryState, getUser(session).getUserId());
			}
