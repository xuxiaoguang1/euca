    ...
    try {
    	return new ArrayList<QuickLinkTag>( Arrays.asList( 
              ...
              new QuickLinkTag( "Resource Management",
                               new ArrayList<QuickLink>( Arrays.asList(
                            		   		  new QuickLink( "虚拟机管理", "desc", "instance",
                                                                QueryBuilder.get( ).start( QueryType.instance ).query( ) ),
                            		          new QuickLink( "镜像管理", "Virtual machine images (EMIs)", "image",
                                                                QueryBuilder.get( ).start( QueryType.image ).query( ) ),
                             		          new QuickLink( "节点控制器管理", "desc", "nodeCtrl",
                                                                QueryBuilder.get( ).start( QueryType.nodeCtrl ).query( ) ),                                                                
                             		          new QuickLink( "集群控制器管理", "desc", "clusterCtrl",
                                                                QueryBuilder.get( ).start( QueryType.clusterCtrl ).query( ) ),                                                                
                               		          new QuickLink( "存储控制器管理", "desc", "storageCtrl",
                                                                QueryBuilder.get( ).start( QueryType.storageCtrl ).query( ) ),                                                                
                             		          new QuickLink( "Walrus控制器管理", "desc", "walrusCtrl",
                             		        		  			QueryBuilder.get( ).start( QueryType.walrusCtrl ).query( ) ),                                                                
